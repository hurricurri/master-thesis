...
\begin{equation*}
    \|\vert \Tilde{u}_i\rangle - U \vert u_i\rangle\|_2 \leq \sqrt{n\epsilon},
\end{equation*}
...
\begin{theorem}[Odd $n$-cycle KS non-contextuality inequalities facilitate self-testing CITE]
...
\end{equation*}where $B_q(\mathcal{G}_{ex}^{(n)})$ is the quantum supremum \ref{eqn:ncyclequsup}, are $\mathcal{O}(\sqrt{\epsilon})$ close in 2-norm to \ref{eqn:ncycleideal}, up to a global isometry:
\begin{align*}
    \exists \emph{ isometry } V & : \| \ket{\Psi} - V \vert u_0^{(n)}\rangle \|_2 \thinspace\leq\thinspace \mathcal{O}(\sqrt{\epsilon})\\[0.7em]
    \forall i\in\{1,\dots,n\} & : \| \Pi_i \ket{\Psi} - V \vert u_i^{(n)} \rangle \langle u_i^{(n)} \vert \thinspace \vert u_0^{(n)} \rangle \| \leq \mathcal{O}(\sqrt{\epsilon})
\end{align*}
\end{theorem}
...
\begin{split}
    ...
    = \thinspace & \| \langle \Tilde{u}_i^{(n)} \vert \Tilde{u}_0^{(n)} \rangle \vert \Tilde{u}_i^{(n)} \rangle - \langle u_i^{(n)} \vert u_0^{(n)} \rangle \thinspace \mathcal{J}^{-1}UV^{'} \vert u_i^{(n)}\rangle\|_2 \leq \mathcal{O}(\sqrt{\epsilon})
\end{split}
\end{align}
...
\begin{figure}
        ...
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[width=0.6\textwidth]{images/allentropies.png}
        ...
        \begin{subfigure}{\textwidth}
            \centering 
            \includegraphics[width=0.6\textwidth]{images/memorycost.png}
