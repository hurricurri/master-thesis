...
& = \sum_{i=1}^{r'}(\Tilde{\sigma}_i-1)^2(\Tilde{\sigma}_i+1)^2 + (r-r')\leq \frac{\epsilon}{\sigma_{\text{min}}^2}\;,
\end{align*}
...
\begin{equation*}
\|\Tilde{V}_2-V_2\|_F = \sum_{i=1}^r(\Tilde{\sigma_i}-1)^2\leq\frac{\epsilon}{\sigma_{\text{min}}^2}\thinspace.
\end{equation*}
...
\end{lemma}
The proof is found in \cite{Bharti2019} (Lemma 9). As $\vert \langle u_i^{(n)}\vert u_0^{(n)} \rangle \vert =5^{\frac{1}{4}}$, the assumption in Lemma \ref{lem:selftestingfinallem} is true for sufficiently small $\epsilon$.
