...
\begin{equation}
\|A-\Tilde{A}\|_F=\|\begin{pmatrix} \mathbb{1_{k'}} \\ 0_{k-k',k'}\end{pmatrix} - \Sigma \|_F = \sqrt{(k'-r)+\sum_{i=1}^{r}(\sigma_i-1)^2}.
\end{equation}
We now use REF to bound the singular values and rank of $A$:
\begin{equation}
\|A^{\dag}A - \mathbb{1} \|_F = \sqrt{(k'-r)+\sum_{i=1}^r(\sigma_i^2-1)^2} \leq \mathcal{O}(d^2\sqrt{\frac{\epsilon}{\mathcal{X}}}),
\end{equation}
where the inequality follows from REF. Thus, for sufficiently small $\epsilon$ the rank of $A$ is $r=k'$, and
\begin{equation}
\|A-\Tilde{A}\|_F=\sqrt{\sum_{i=1}^{k'}(\sigma_i - 1)^2}\leq\sqrt{\sum_{i=1}^{k'}(\sigma_i - 1)^2(\sigma_i+1)^2}\leq\mathcal{O}(d^2\sqrt{\frac{\epsilon}{\mathcal{X}}}).
\end{equation}
