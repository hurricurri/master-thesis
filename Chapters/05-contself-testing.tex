...
\|Z\|_F & \stackrel{(\square)}{\leq}\epsilon\thinspace\lambda_{\text{min}}^{(n)}{}^{-1}+&&\thinspace\mathcal{O}(\epsilon^2\lambda_{\text{min}}^{(n)}{}^{-2})\thinspace,
\end{alignat*}
...
Therefore,
\begin{equation*}
\|B^{\dag}-\Tilde{V}_2\Sigma_r W_1^{\dag}\|_F=\|ZW_2^{\dag}\|_F=\|Z\|_F\leq \epsilon\thinspace \lambda_{\text{min}}^{(n)}{}^{-1}+\mathcal{O}(\epsilon^2\lambda_{\text{min}}^{(n)}{}^{-2}).
\end{equation*}
...
\begin{align*}
\|B^{\dag}-V_2\Sigma_r W_1^{\dag}\|_F\leq \|B^{\dag}-\Tilde{V_2}\Sigma_r W_1^{\dag}\|_F + \|(\Tilde{V}_2-V_2)\Sigma_r W_1^{\dag}\|_F & \leq \epsilon\thinspace\lambda_{\text{min}}^{(n)}{}^{-1}(1+\sigma_{\text{max}}^{(n)})\thinspace + \mathcal{O}(\epsilon^2\lambda_{\text{min}}^{(n)}{}^{-2}),
\end{align*}
where $\sigma_{\text{max}}^{(n)}$ is the maximum singular value of $A^{\dag}$. We take the second term to be negligible. For the KCBS scenario, $n=5+1$, $\sigma_{\text{max}}^{(n)}=\sqrt{2}$, and consequently $\|B^{\dag}-V_2\Sigma_r W_1^{\dag}\|_F\leq \kappa\epsilon$, where $\kappa \approx 3.55$. Further study is needed to find how $\kappa(n)$ scales with $n$, in order to see how this bound compares to that in \cite{Bharti2019} for very large cycle lengths.
...
\begin{equation*}
\forall i\in\{1,\dots,n\}: \|\Tilde{u}_i-U u_i\|_2 \leq \kappa(n)\epsilon + \mathcal{O}(\kappa(n)^2\epsilon^2).
\end{equation*}
...
Let us go through Assumptions 1-6 in order:
...
For a second, assume that Assumption 2 is not true, and consider the preparation and  measurement devices to be correlated. We express this in terms of the shared random variable $\lambda$. As noted in CITE, such shared randomness can render the task of self-testing futile. Consider a quantum experiment where, based on the random variable $\lambda$, the preparation device prepares $\rho_\lambda=U_\lambda \rho \thinspace U_\lambda^{\dag}$ and the measurement $i$ is characterized by the positive semi-definite operator $M_\lambda^{(i)}=U_\lambda M^{(i)} U_\lambda^{\dag}$. The probability $p(0,1 \thinspace \vert \thinspace i,i\oplus 1)=\int_{\Lambda} d\lambda\thinspace\mu(\lambda)\operatorname{tr}(\rho_\lambda M_\lambda^{(i)})=\operatorname{tr}(\rho M^{(i)})$. The correlation experiment with shared randomness is therefore operationally indistinguishable from one without, and we cannot hope to relate the physical experiment to the reference experiment via a single unitary operator, based on observed correlations. Additionally, to bound classical simulations reproducing the statistics in a non-trivial way, we will model these as information transducers that take a freely random input from the verifier at every step of the input-output cycle. 
