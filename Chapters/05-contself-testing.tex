...
We will omit the proofs of Lemmas \ref{lem:kcbsunique} and \ref{lem:epssuboptgram}, as they do not provide much conceptual insight, and offer only a few comments. We include the proof of Lemma \ref{lem:closegramdecomp}, as \cite{Bharti2019} proves only a weaker version for $d=n$, but to the best of the author's knowledge implicitly uses a stronger statement analogous to that of Lemma \ref{lem:closegramdecomp}. Furthermore, the proof we present establishes a tighter error bound of the order $\mathcal{O}(n\epsilon)$, compared to $\mathcal{O}(\sqrt{n\epsilon})$ in \cite{Bharti2019}, the only drawback being that this bound no longer scales sub-linearly in $n$, which might be relevant for very large cycle lengths.
...
& 0_{n-r}
\end{pmatrix}
\right\|_F\;=\;\|WD^{-1}W^{\dag}(\Tilde{X}-X^*)WD^{-1}W^{\dag}\|_F\; \leq\; \frac{\epsilon}{\sigma_{\text{min}}^2}\thinspace.
\end{equation*}
...
\end{alignat*}
where the inequality $(\square)$ follows from the norm equivalence $\|\vec{x}\|_2\leq \|\vec{x}\|_1\leq \sqrt{k}\thinspace\|\vec{x}\|_2$, for $\vec{x}\in\mathbb{C}^k$. 
...
B^{\dag} & \stackrel{\mathcal{O}(n\epsilon)}{\approx} V_2\Sigma_r W_1^{\dag},
\end{align*}
...
\begin{equation*}
B^{\dag}\stackrel{\mathcal{O}(n\epsilon)}{\approx}V_2\Sigma_r W_1^{\dag}= UV_1\Sigma_r W_1^{\dag}=UA^{\dag}.
\end{equation*} 
...
\begin{equation*}
\forall i\in\{1,\dots,n\}: \|\Tilde{u}_i-U u_i\|_2 \leq \mathcal{O}(n\epsilon).
\end{equation*}
\end{proof}
